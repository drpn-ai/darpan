...
| `darpan.reconciliation.automation.maxConcurrentExecutions` | 4 | Max automation executions dispatched concurrently per scan (bounds shared service-pool and Spark-driver load). |
| `darpan.reconciliation.automation.executionTimeoutSeconds` | 1800 | Per-execution wait cap in the scan loop; a hung execution is cancelled rather than stalling the rest. |
