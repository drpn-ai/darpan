...
Manual saved-run API execution accepts the selected calendar dates along with the timestamp payload. The active tenant time zone defines the requested reconciliation period, and Darpan converts that tenant-local period to exact instants before source extraction. Source-specific order APIs then receive those same instants in the representation their API expects. Shopify receives UTC `created_at` bounds. HotWax receives epoch milliseconds as `orderDate_from` and `orderDate_thru`, which represent the equivalent local instants in the configured OMS time zone. For example, a tenant in `America/New_York` selecting `2026-05-04` through `2026-05-05` is dispatched as `2026-05-04T04:00:00Z` through `2026-05-05T04:00:00Z` to both systems, even if Shopify is configured as `UTC` and OMS is configured as `America/Los_Angeles`.
...
1. Shopify orders are extracted with a GraphQL `orders(first: 100, after: $after, query: $search, sortKey: CREATED_AT)` query using quoted UTC bounds, for example `created_at:>='<windowStartUtc>' created_at:<'<windowEndUtc>'`, where the UTC window is derived from the tenant reconciliation period. If Shopify still has another cursor page after the per-window page guard, Darpan splits the same selected `created_at` window into smaller half-open ranges and continues extraction.
2. HotWax OMS orders are extracted with `orderDate_from` and `orderDate_thru` epoch milliseconds for the same tenant-derived instants.
