...
This document defines the Darpan permission contract for tenant data, platform administration, and user-visible page/action access.
...
| --- | --- | --- | --- |
| Super Admin | Platform-wide. This is the ultimate Darpan administrator. | Can access every configured Darpan tenant. Tenant-owned operational pages still use an active tenant unless a private admin screen intentionally aggregates across tenants. | Can view and edit all tenant data, all tenant setup, all Darpan core/platform settings, and private Darpan management screens above tenant administration. |
...
The product vocabulary is three roles: `DARPAN_SUPER_ADMIN`, `DARPAN_TENANT_ADMIN`, and `DARPAN_TENANT_USER`.
...
- Tenant permission seed data includes `DARPAN_TENANT_ADMIN` and `DARPAN_TENANT_USER` under `UgtDarpanPermission`.
...
| Super-admin access | `UserGroupMember(userGroupId="ADMIN")` or `UserGroupMember(userGroupId="DARPAN_SUPER_ADMIN")` | Grants platform-wide Darpan administration and private Darpan management access. |
...
| Permission group type | `moqui.basic.Enumeration(enumId="UgtDarpanPermission")` | Groups reusable Darpan tenant permission levels. |
...
| --- | --- | --- |
| Super Admin | `UserGroupMember(userId, "ADMIN")`, or both `UserGroupMember(userId, "DARPAN_SUPER_ADMIN")` and `UserGroupMember(userId, "DARPAN_USER")` for PWA facade access. | Does not require per-tenant `TenantUserPermissionGroupMember` rows. Super Admin can see every tenant and can make Darpan core/platform changes. |
...
| View/edit AI/LLM provider settings | Yes | No | No |
| View/edit enum/global system settings | Yes | No | No |
| Use debug, diagnostics, migration, or Darpan core management screens | Yes | No | No |
...
| AI/LLM settings | View/edit. | No access. | No access. |
| Global enum/system settings | View/edit. | No access. | No access. |
| Tenant/user/permission management | View/edit. | No access. | No access. |
| Debug and operational diagnostics | View/edit/use. | No access unless a future tenant-safe diagnostic surface is explicitly created. | No access unless a future tenant-safe diagnostic surface is explicitly created. |

## Effective Access Resolution

1. Resolve whether the user is a Super Admin first. Super Admin can view and edit everything in Darpan and can access private platform-management screens.
...
| Field | Meaning |
| --- | --- |
| `isSuperAdmin` | User has platform-wide Darpan administration. |
...
| `canManageDarpanCore` | Can access global Darpan platform settings and private management screens. |
