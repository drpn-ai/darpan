...
// Audit M5.4 — reconciliation file uploads (JSON/CSV/JSONL) were size-unbounded; a multi-GB upload
// would fill the runtime data-manager volume. Cap defensively. Schema files / order extracts are
// well under 500 MB in normal operation. Override via the darpan.upload.reconciliation.maxBytes
// resource property if a tenant legitimately needs more.
def __MAX_UPLOAD_BYTES_DEFAULT = 500L * 1024L * 1024L
def __maxUploadBytesProp = ec?.resource?.properties?.get("darpan.upload.reconciliation.maxBytes")?.toString()?.trim()
def maxUploadBytes = (__maxUploadBytesProp && __maxUploadBytesProp.isLong()) ?
        Long.parseLong(__maxUploadBytesProp) : __MAX_UPLOAD_BYTES_DEFAULT

def saveFileItem = { fileItem, targetRef ->
    long incomingSize = 0L
    try { incomingSize = (fileItem?.getSize() ?: 0L) as long } catch (Exception ignored) { incomingSize = 0L }
    if (incomingSize > maxUploadBytes) {
        throw new IllegalArgumentException("Uploaded reconciliation file exceeds the ${maxUploadBytes >> 20} MB limit.")
    }
...
def saveTextPayload = { String payload, targetRef ->
    ...
    // Security (HIGH gap 4): file1Text/file2Text are caller-supplied strings already materialized in the
    // heap. Enforce the same byte cap as saveFileItem so a multi-GB payload cannot OOM the shared JVM or
    // fill the data-manager volume. Normal text/CSV/JSON payloads are far under the 500 MB default.
    if (bytes.length > maxUploadBytes) {
        throw new IllegalArgumentException("Reconciliation text payload exceeds the ${maxUploadBytes >> 20} MB limit.")
    }
    File targetFile = targetRef.getFile()
    if (targetFile != null) {
...
def resolveMappingName = { String mappingId ->
    ...
    // Directly-owned: gate via findTenantScopedByIdQuiet (null on denial, no ec.message error).
    def mapping = TenantScopedFinder.findTenantScopedByIdQuiet(
            ec, DarpanEntityConstants.RECONCILIATION_MAPPING, "reconciliationMappingId", normalized)
...
def resolveRuleSet = { String ruleSetId ->
    ...
    if (!normalized) return null
    // Directly-owned: gate via findTenantScopedByIdQuiet (null on denial, no ec.message error).
    return TenantScopedFinder.findTenantScopedByIdQuiet(
            ec, DarpanEntityConstants.RULE_SET, "ruleSetId", normalized)
}
def persistRunResult = { Map<String, Object> fields ->
    ...
    return ec.transaction.runUseOrBegin(30, "Error saving reconciliation run result", {
        def runResultValue = null
        if (existingRunResultId) {
            // P0 #4 step 2 — gate ReconciliationRunResult by active tenant; fail closed on foreign/missing.
            // Use the quiet variant so no ec.message error is added — the denial is expected and
            // handled here (create-fresh fallback), and we must not pollute the caller's error state.
            runResultValue = TenantScopedFinder.findTenantScopedByIdQuiet(
                    ec, DarpanEntityConstants.RECONCILIATION_RUN_RESULT,
                    "reconciliationRunResultId", existingRunResultId)
            if (runResultValue == null) {
                // Foreign or missing — do NOT read or update. Fail closed. No clearErrors() needed.
                logger.warn("[P0#4] persistRunResult: denied foreign/missing reconciliationRunResultId '{}' — fail closed, tracking reset", existingRunResultId)
                return null
            }
        }
...
// Security (HIGH gaps 3,5): IGNORE any caller-supplied sparkMaster. Resolving it server-side only
// prevents an authenticated API caller (Postman) from redirecting the tenant's Spark job — and the
// staged file1/file2 financial data — to an attacker-controlled cluster. Operators set spark.master
// via the server property; the PWA never sends this field, so no legitimate input is rejected.
String sparkMasterToUse = (ec.resource.properties["spark.master"] ?: "local[*]")
...
if (ruleSetIdValue) {
    ...
    // Audit 2026-06-11 #16: hold the persisted Spark Datasets so they are unpersisted in the finally
    // below on every exit path (success or failure) — no persisted block survives the run.
    ...
    try {
        ...
        // Audit 2026-06-11 #4: a rule build/eval failure does not throw (it is caught and returned as
        // an error flag), so without this the run would persist as SUCCEEDED despite the ruleset not
        // fully evaluating. Record the run as FAILED so the status and completion alert reflect it.
        ...
        reconciliationRunResultId = persistRunResult([
                ...
                statusEnumId             : ruleExecutionFailed ? ReconciliationOutputSupport.STATUS_FAILED : ReconciliationOutputSupport.STATUS_SUCCEEDED,
        ...
        if (ruleExecutionFailed) {
            logger.error("Generic reconciliation via RuleSet path recorded FAILED — ruleset did not fully evaluate: ruleSet={} compareScope={} errors={}",
                    ruleSetIdValue, compareScopeConfig.compareScopeId, (reconcileResult.ruleErrors ?: []))
        } else {
        ...
        ReconciliationServices.unpersistDatasets(ruleSetPersistedSources)
    }
}
