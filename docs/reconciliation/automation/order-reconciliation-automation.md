...
Manual saved-run API execution accepts the selected calendar dates along with the timestamp payload. Source-specific order APIs normalize those calendar dates through the selected source config timezone before extraction. Shopify uses `ShopifyAuthConfig.timeZone` before creating UTC GraphQL filters. HotWax uses `HotWaxOmsRestSourceConfig.timeZone` before sending epoch milliseconds as `orderDate_from` and `orderDate_thru`. For example, a UI-selected `2026-03-01` through `2026-04-01` window for a source config in `America/Chicago` is dispatched as `2026-03-01T06:00:00Z` through `2026-04-01T05:00:00Z`, preserving the source-local month across DST.
