...
class LlmSettingsSupport {
    ...
    static Map<String, Object> buildSavePayload(def ec, Object rawProvider, Object llmApiKey, Object llmModel,
        ...
        String baseUrlToUse = normalize(llmBaseUrl) ?:
                ...
                defaults.baseUrl
        // Security (HIGH gaps 7,8): baseUrlToUse is persisted as SystemMessageRemote.sendUrl and later
        // receives the API key + DRL prompt on every generation call. An authenticated admin could
        // otherwise set http://169.254.169.254/... (IMDS credential harvest) or http://internal-db:5432
        // (internal probe). Validate via the existing SSRF helper, mirroring SettingsFacadeServices.xml
        // :514 / :657. Allow-list is the two known LLM provider hosts; the legit PWA only ever sends one
        // of those https URLs (or leaves it blank to keep the default), so no legitimate input is lost.
        def __baseUrlCheck = OutboundHttpPolicy.validate(baseUrlToUse, ['api.openai.com', 'generativelanguage.googleapis.com'])
        if (!__baseUrlCheck.ok) {
            ec.message.addError("LLM Base URL is not allowed: ${__baseUrlCheck.error}".toString())
            return [:]
        }
