...
- The component does not expose tenant-management facade services (`list#Tenants`,
  `save#Tenant`, `save#TenantUser`, and `save#TenantUserPermissionGroup` do not exist).
  Tenant setup creates `moqui.security.UserGroup`, `moqui.security.UserGroupMember`, and
  `darpan.auth.TenantUserPermissionGroupMember` records directly through the generic-data setup
  path in [Darpan Tenant Setup Tutorial](tenant-setup.md); Darpan does not maintain a separate
  company entity.
