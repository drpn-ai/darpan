...
> **Status (2026-06): runtime cutover shipped; Mapping migration not landed.**
> The RuleSet compare-scope runtime path is in production: `RuleSetCompareScope` /
> `RuleSetCompareSource` entities, `reconciliation.ReconciliationCoreServices.prepare#RuleSetCompareScope`,
> `reconcile#RuleSetCompareScopeBaseDiff`, `reconcile#RuleSetCompareScope`, and the
> `facade.ReconciliationFacadeServices.create#RuleSetRun` / `save#RuleSetRun` contracts all exist
> and execute (Waves 1–3). The Wave 4 Mapping migration/deprecation ticket (RSCUT-009, the
> planned `migrate#MappingsToRuleSetScopes` service) was never implemented, so Mapping-backed
> runs remain a supported legacy bridge through `reconcile#FilesByMapping`. Treat the rest of
> this document as the historical project plan, not active work.
