...
- Current upgrade data for customer/self-hosted upgrades: `runtime/component/darpan/data/upgrade-data.xml`, loaded in Docker startup with MoquiStart `load` using the `darpan-seed` reader type and `component://darpan/data/upgrade-data.xml` location
