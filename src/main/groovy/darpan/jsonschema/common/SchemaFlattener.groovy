...
class SchemaFlattener {
    ...
    private static Map<String, Object> pickProperties(Map<String, Object> node) {
        // Use explicit map access: Groovy resolves `.properties` to the bean getProperties(), not the map entry.
        if (node.get('properties') instanceof Map) return (Map<String, Object>) node.get('properties')
        ...
        Object validOption = options.find { it instanceof Map && ((Map) it).get('properties') instanceof Map }
        return validOption ? (Map<String, Object>) ((Map) validOption).get('properties') : null
    }
}
